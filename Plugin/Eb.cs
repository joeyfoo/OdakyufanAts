...
namespace Plugin {
	...
	internal class Eb : Device {
		...
		internal override void Elapse(ElapseData data, ref bool blocking) {
			// --- behavior ---
			if (!blocking) {
				if (Math.Abs(data.Vehicle.Speed.MetersPerSecond) > SpeedThreshold | this.Counter >= TimeUntilBell) {
					this.Counter += data.ElapsedTime.Seconds;
					if (this.Counter >= TimeUntilBrake) {
						if (this.Train.AtsSx != null && this.Train.AtsSx.State != AtsSx.States.Disabled) {
