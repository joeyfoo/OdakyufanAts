...
namespace Plugin {
	...
	internal class AtsPs : Device {
		...
		internal enum States {
			...
			Emergency = 6,
			/// <summary>The system has been deactivated by a beacon combination.</summary>
			Deactivated = 7
		}
		...
		private class FunctionSelector {
			...
			internal void Perform(AtsPs system, ElapseData data) {
				if (this.FirstBeaconFrequency == 90 && this.SecondBeaconFrequency == 108) {
					// --- Ps activate/deactivate ---
					if (this.DistanceAccumulator > 1.0 & this.DistanceAccumulator < 2.7) {
						if (system.State == States.Deactivated) {
							system.State = States.Normal;
							system.Train.Sounds.AtsPsPatternEstablishment.Play();
						}
					} else if (this.DistanceAccumulator > 3.3 & this.DistanceAccumulator < 4.8) {
						system.State = States.Deactivated;
						foreach (var pattern in system.Patterns) {
							pattern.Clear();
						}
						system.Train.Sounds.AtsPsPatternRelease.Play();
					}
					this.Clear();
				}
				if (this.FirstBeaconFrequency != 0 & system.State != States.Deactivated) {
				...
				if (this.FirstBeaconFrequency != 0) {
					...
					if (this.DistanceAccumulator > 25.0) {
						this.Clear();
					}
				}
			}
		...
		private void SwitchToPs() {
			if (this.State == States.Standby) {
				...
				this.Train.Sounds.AtsPsPatternEstablishment.Play();
			}
		}
		...
		private void SwitchToSx() {
			if (this.State == States.Emergency) {
				...
				this.State = States.Standby;
				this.Train.Sounds.AtsPsPatternRelease.Play();
			}
			this.Selector.Clear();
		...
		internal override void Elapse(ElapseData data, ref bool blocking) {
			...
			if (blocking) {
				...
				if (this.State != States.Standby & this.State != States.Deactivated) {
					foreach (var pattern in this.Patterns) {
					...
					if (Math.Abs(data.Vehicle.Speed.MetersPerSecond) >= limit) {
						if (this.State != States.Emergency) {
							...
							this.Train.Sounds.AtsPsChime.Play();
						}
			...
			// --- panel ---
			if (this.State != States.Disabled & this.State != States.Suppressed & this.State != States.Standby & this.State != States.Deactivated) {
				double limit = double.MaxValue;
			...
			if (this.State == States.Normal | this.State == States.Pattern | this.State == States.Approaching | this.State == States.Emergency) {
				...
				this.TrainPermanentPattern.AddToStringBuilder("最高/Max:", builder);
				if (builder.Length == 0) {
		...
		internal override void SetBeacon(BeaconData beacon) {
			if (this.State != States.Disabled) {
				...
				if (frequency != 0) {
					...
					return;
				} else if (this.State == States.Deactivated) {
					return;
				}
				// --- legacy beacons ---
				switch (beacon.Type) {
